...
```{r, warning = FALSE}
dat <- read_csv("../data/latency-condition.csv")
```
...
Test whether latencies are different when opening compartments during test trials, when a pig was trapped in the test compartment, and during the preceding separation, when both compartments were empty.
